...
/* ============================================================
   9b. GROUP PORTRAIT MODAL (keyword: "fathir")
============================================================ */
function renderGroupPortrait() {
  ...
  `;
  const freshmenFaces = Object.values(LETTERS_DATA).map((f) => `
    <div class="group-face">
      <img src="${f.photo}" alt="${f.name}">
      <span>${f.name.split(" ")[0]}</span>
    </div>
  `).join("");
  facesContainer.innerHTML = leaderFace + freshmenFaces;

  const memoriesContainer = document.getElementById("groupMemories");
  memoriesContainer.innerHTML = GROUP_DATA.memories.map((m) => `
    <div class="group-memory-thumb">
...
function initGroupModal() {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && modal.classList.contains("open")) closeGroupModal();
  });
}
